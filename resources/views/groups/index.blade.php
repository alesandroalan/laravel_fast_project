            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-end mb-3 mt-3">
                        {!! Form::open(['method' => 'GET' ,'route' => ['groups.index']]) !!}
                        ...
                        <table class="table table-bordered table-sm table-hover">
                            ...
                            @foreach($groups as $group)
                                ...
                                <tr>
                                    <td>{{ $group->id }}</td>
                                    <td>{{ $group->name }}</td>
                                    <td>{{ \App\Models\User::with('roles')->get()->filter(
    fn ($user) => $user->roles->where('id', $group->id)->toArray()
)->count() }}</td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('groups.edit', [$group->id]) }}"
                                               class="btn btn-primary btn-xs">{{__('adminlte::main.Edit')}}</a>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            {!! Form::open(['method' => 'DELETE', 'name' => 'delete' ,'route' => ['groups.destroy', $group->id]]) !!}
                        ...
                        <small>{{__('adminlte::main.Showing')}} {{$groups->firstItem() ?? "0"}} {{__('adminlte::main.to')}} {{$groups->lastItem() ?? "0"}} {{__('adminlte::main.of')}} {{$groups->total() ?? "0"}} {{__('adminlte::main.items')}}</small>
                    </div>
                    {{$groups->links()}}
                </div>
            </div>
