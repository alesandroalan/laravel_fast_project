...
class MakeViewsService
{
    ...
    public function findAndReplaceIndexViewPlaceholderColumns($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions, $withLivewire, $columnInSearch)
    {
        ...
        foreach ($columns as $column)
        {
            ...
            // our placeholders
            $thIndex    .=str_repeat("\t", 4)."<th>{{__('adminlte::main.".trim($column)."')}}</th>\n";
        ...
        $indexStub = str_replace('DummyPageTitle', "__('adminlte::main.".$namingConvention['plural_low_name']."')", $indexStub);
        $indexStub = str_replace('DummyPageSectionTitle', "{{__('adminlte::main.".$namingConvention['plural_low_name']."')}}", $indexStub);
    ...
    public function findAndReplaceCreateViewPlaceholderColumns($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        ...
        foreach ($columns as $column)
        {
            ...
            $formCreate .=str_repeat("\t", 3).'{{ Form::label(\''.trim($column).'\', __(\'adminlte::main.'.ucfirst(trim($column)).'\'), [\'class\'=>\'form-label\']) }}'."\n";
        ...
        $createStub = str_replace('DummyPageTitle', "__('adminlte::main.".$namingConvention['plural_low_name']."')", $createStub);
        $createStub = str_replace('DummyPageSectionTitle', "{{__('adminlte::main.".$namingConvention['plural_low_name']."')}}", $createStub);
        return $createStub;
    }
    ...
    public function findAndReplaceEditViewPlaceholderColumns($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        ...
        foreach ($columns as $column)
        {
            ...
            $formEdit .=str_repeat("\t", 3).'{{ Form::label(\''.trim($column).'\', __(\'adminlte::main.'.ucfirst(trim($column)).'\'), [\'class\'=>\'form-label\']) }}'."\n";
        ...
        $editStub = str_replace('DummyPageTitle', "__('adminlte::main.".$namingConvention['plural_low_name']."')", $editStub);
        $editStub = str_replace('DummyPageSectionTitle', "{{__('adminlte::main.".$namingConvention['plural_low_name']."')}}", $editStub);
        return $editStub;
    }
